...
inner join products on products.id=orders.product_id
where products.name='Filter'
and orders.order_time between '2017-01-15' and '2017-02-14'
order by orders.order_time;
...
select * from customers;
 ...
 -- select the product name and the price and order time for all the orders from femails in january 2017.*/
 
select p.name,p.price,o.order_time from products p
join orders o on p.id=o.product_id
join customers c on o.customer_id=c.id
where c.gender='F' and order_time between '2017-01-01' and '2017-01-30'
order by o.order_time;
